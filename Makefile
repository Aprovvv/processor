...
RFLAGS=-D NDEBUG
SOURCES=main.cpp stack/stack.cpp stack/color_print/color_print.cpp
ASOURCES=ass.cpp stack/stack.cpp stack/color_print/color_print.cpp
AOBJECTS=$(addprefix ass_object_files/,$(notdir $(ASOURCES:.cpp=.o)))
...
debug_object_files/%.o: stack/%.cpp#добавлено из-за того, что stack в отдельной папке
	$(CC) -c $(DFLAGS) $< -o $@

debug_object_files/%.o: stack/color_print/%.cpp#добавлено из-за того, что color_print в отдельной папке
	$(CC) -c $(DFLAGS) $< -o $@
...
ass: $(SOURCES) $(DEXECUTABLE)

$(AEXECUTABLE): $(AOBJECTS)
	$(CC) $(DFLAGS) $(AOBJECTS) -o $@

ass_object_files/%.o: %.cpp
	$(CC) -c $(DFLAGS) $< -o $@

ass_object_files/%.o: stack/%.cpp#добавлено из-за того, что stack в отдельной папке
	$(CC) -c $(DFLAGS) $< -o $@

ass_object_files/%.o: stack/color_print/%.cpp#добавлено из-за того, что color_print в отдельной папке
	$(CC) -c $(DFLAGS) $< -o $@
